...
describe('Algebra provider tests', () => {
    ...
    test('Must calculate correct NATIVE-ERC20 trade with simple path.', async () => {
        ...
        expect(trade.path[1].address).toBe(TOKENS.USDC_POLYGON.address);
        expect(trade.path[2].address).toBe(TOKENS.USDT_POLYGON.address);
    }, 300_000);
    ...
    test('Must calculate correct ERC20-ERC20 trade with simple path.', async () => {
        ...
        const to = await PriceToken.createFromToken(TOKENS.USDC_POLYGON);
